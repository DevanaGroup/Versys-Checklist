...
const Login = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (email && password) {
      localStorage.setItem("versys_user", JSON.stringify({ email, name: "João Silva" }));
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-versys-primary to-versys-secondary p-4">
      <Card className="w-full max-w-md shadow-2xl border-0">
        <CardHeader className="text-center pb-2">
          ...
          <p className="text-sm text-gray-600">Segurança Portuária</p>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <Button 
              type="submit" 
              className="w-full bg-versys-primary hover:bg-versys-secondary transition-colors"
            >
              Fazer Login
            </Button>
            <div className="text-center">
              <button 
                type="button"
                className="text-sm text-versys-secondary hover:text-versys-primary transition-colors"
              >
                Esqueci minha senha
              </button>
            </div>
